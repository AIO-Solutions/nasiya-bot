...
async def getname(message : types.Message, state : FSMContext):
    ...
    await message.answer(f"<i>{message.text}</i> iltimos telefo'n raqamingizni kiriting yoki kontakni ulashish tugmasni bosing", 
                         reply_markup = menu.phone_number(),
                         parse_mode = 'html')
...
async def getnumber(message : types.Message, state : FSMContext):
    if message.text == "⬅️ Orqaga":
        ...
        await message.answer("Ilimos ismingizni qaytadan kiriting", reply_markup = types.ReplyKeyboardRemove())
